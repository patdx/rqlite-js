...
describe('http-request', () => {
    ...
    RETRYABLE_HTTP_METHODS.forEach((httpMethod) => {
      RETRYABLE_STATUS_CODES.forEach((statusCode) => {
        it(`should return true for HTTP method ${httpMethod} and status code ${statusCode}`, () => {
          assert.isTrue(
            httpRequest.requestIsRetryable({
              statusCode,
              httpMethod,
            })
          );
  ...
  describe('Method: HttpRequest.get()', () => {
    ...
    RETRYABLE_STATUS_CODES.forEach((statusCode) => {
      it.skip(`should perform a HTTP GET request and retry using the next host on retryable http status code ${statusCode}`, async () => {
        ...
        assert.isTrue(
          scopeSuccess.isDone(),
          'http success request captured by nock'
        );
        assert.isTrue(
          scopeFailure.isDone(),
          'http failure request captured by nock'
        );
        assert.deepEqual(QUERY_SUCCESS_RESPONSE, res.body);
      });
      it.skip(`should perform a HTTP GET request, retry using the next host on retryable http status code ${statusCode} and reject on max retries`, async () => {
        ...
        scopes.forEach((scope, i) => {
          assert.isTrue(
            scope.isDone(),
            `http failure request attempt ${i} captured by nock`
          );
        });
      });
    });
    it(`should perform a HTTP GET request and not retry on http status code ${HTTP_STATUS_CODE_NOT_RETRYABLE}`, async () => {
      ...
      assert.isFalse(
        scopeSuccess.isDone(),
        'http success request captured by nock, but not performed'
      );
      assert.isTrue(
        scopeFailure.isDone(),
        'http failure request captured by nock'
    ...
    it('should perform a HTTP GET request with a query and follow redirects', async () => {
      ...
      assert.isTrue(
        ...
        'http redirect request captured by nock'
      );
      assert.isTrue(scope.isDone(), 'http request captured by nock');
      assert.deepEqual(QUERY_SUCCESS_RESPONSE, res.body);
    });
  ...
  describe('Method: HttpRequest.post()', () => {
    ...
    it(`should send a HTTP POST request including ${CONTENT_TYPE_APPLICATION_JSON} body and not retry on http status code ${HTTP_STATUS_CODE_NOT_RETRYABLE}`, async () => {
      ...
      assert.isFalse(
        scopeSuccess.isDone(),
        'http success request captured by nock, but not called'
      );
      assert.isTrue(
        scopeFailure.isDone(),
        'http failure request captured by nock'
    ...
    it(`should send a HTTP POST request including ${CONTENT_TYPE_APPLICATION_JSON} body and follow redirects when useLeader is true plus redirect host is in host list`, async () => {
      ...
      const httpRequest = new HttpRequest(
        `${url},${urlRedirectDestination}${path}`
      );
      const body = ['INSERT INTO foo(name) VALUES("fiona")'];
      const scopeRedirect = executeRedirectSuccess({
        url,
      ...
      assert.deepEqual(res.body, EXECUTE_SUCCESS_RESPONSE);
      assert.equal(
        httpRequest.getLeaderHostIndex(),
        1,
        'leader host index matches redirect'
      );
    });
